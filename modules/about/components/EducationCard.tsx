// File: src/modules/about/components/EducationCard.tsx
    ...
    <SpotlightCard className="flex flex-col items-start gap-5 p-6 md:flex-row">
      ...
      <div className="w-full space-y-1">
        <a href={link || "#"} target="_blank" className="hover:underline">
          <h6 className="font-semibold text-neutral-800 dark:text-neutral-50">{school}</h6>
        </a>
        <div className="space-y-2 text-sm text-neutral-600 dark:text-neutral-400">
          <div className="flex flex-col gap-1 md:flex-row md:gap-2">
            <span>{degree[locale as keyof MultiLanguageText]}</span>
            <span className="hidden text-neutral-300 dark:text-neutral-700 md:block">•</span>
            <span>{major[locale as keyof MultiLanguageText]}</span>
          </div>
        ...
        {responsibilities && responsibilities[locale as keyof MultiLanguageResponsibilities] && (
          <div className="mt-4">
            ...
            >
              ...
              {/* Mengganti teks agar lebih sesuai dengan konteks pendidikan */}
              <span>
                {isOpen ? 'Hide Activities & Achievements' : 'Show Activities & Achievements'}
              </span>
            </button>
